...
const Transactions = () => {
  ...
  const [categoryInputVlue, setCategoryInputValue] = useState<string>("");
  useEffect(() => {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<inputType>({ resolver: yupResolver(addTransactionSchema) });
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (categoryFilter && categoryFilter !== "all") {
        filtered = filtered.filter((item:Transaction) => item.category === categoryFilter);
      }

      if (typeFilter && typeFilter !== "all") {
        filtered = filtered.filter((item:Transaction) => item.type === typeFilter);
      }
      if (costFilter) {
        ...
        if (costRange.length === 2) {
          ...
          filtered = filtered.filter(
            (item:Transaction) => item.cost >= minCost && item.cost <= maxCost
          );
        }
      }
  ...
  const onSubmit = ({ category, cost, type, customCategory }:inputType) => {
    setModalStatus(false);
    if (customCategory) {
      categoriesDispatch({ type: ADD, payload: customCategory });
    }
    console.log("values:", getValues());
    reset({ category: "", type: "expense", cost: "" });
              ...
              <Controller
                ...
                render={({ field }) => (
                  <CustomInput
                    ...
                    title="category"
                    type="select"
                    selectiontArray={["", ...categories, "other..."]}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <CustomInput
                    ...
                    title="type"
                    type="select"
                    selectiontArray={["", "income", "expense"]}
